...
namespace ServiceWebsite.Web.Areas.Admin.Controllers
{
    ...
    public class ReferenceSettingController : AdminBaseController
    {
        ...
        {
            ViewData.TemplateInfo = new TemplateInfo { HtmlFieldPrefix = "ReferenceAdd" };

            return PartialView("~/Areas/Admin/Views/ReferenceSetting/_ReferenceAdd.cshtml");
        }
        [HttpPost, ValidateInput(false), ValidateAntiForgeryToken]
        public async Task<ActionResult> Add([Bind(Prefix = "ReferenceAdd")] ReferenceAddViewModel model)
        {
        ...
        public async Task<ActionResult> Edit(int referenceId)
        {
            ...
            if (model != null)
            {
                ViewData.TemplateInfo = new TemplateInfo { HtmlFieldPrefix = "ReferenceEdit" };
        ...
        [HttpPost, ValidateInput(false), ValidateAntiForgeryToken]
        public async Task<ActionResult> Edit([Bind(Prefix = "ReferenceEdit")] ReferenceEditViewModel model)
        {
            HttpFileCollectionBase Files = Request.Files;
            HttpPostedFileBase ImageFile = Files[0];
            if (ImageFile != null && ImageFile.ContentLength > 0)
